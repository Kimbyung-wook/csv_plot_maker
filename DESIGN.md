...
- **"Remove All" 버튼** *(2026-08-28 추가)*: 기존에는 키워드를 하나씩만("Remove Selected") 지울 수 있어 목록이 길면 불편했음. "Remove Selected" 옆에 "Remove All"을 추가하고, 실수로 전체를 날리는 사고를 막기 위해 개수를 보여주는 확인창(`QMessageBox.question`)을 거친 뒤에만 비움 — 목록이 이미 비어 있으면 확인창 없이 그냥 무시. 여기서도 디스크엔 아무것도 쓰지 않음(`HeaderTrimDialog._on_remove_all`)
...
### 3.6 데이터/시리즈 리스트 선택 및 이동 *(2026-09-09 추가, 우측 리스트 드래그 이동으로 재수정)*
- **우측 시리즈 리스트(현재 subplot의 Y series)에 Ctrl/Shift 다중 선택 적용**: 기존에는 `QListWidget` 기본값(단일 선택)이라 한 번에 하나만 고를 수 있었음. Data 탭의 컬럼 리스트(`DraggableColumnList`)와 동일하게 `ExtendedSelection`으로 바꿔 Ctrl-클릭으로 개별 토글, Shift-클릭으로 범위 선택이 되도록 함(`SeriesListWidget`). Delete/Backspace도 이제 "선택된 항목 전체"에 대해 동작 — 삭제 직전에 선택된 항목들의 id를 먼저 문자열로 뽑아두고 하나씩 `series_delete_requested`를 발생시킴(각 emit이 동기적으로 `MainWindow._remove_series`를 거쳐 리스트를 다시 그리므로, 나중 항목을 가리키는 `QListWidgetItem` 참조가 아니라 미리 뽑아둔 id 문자열을 순회해야 안전함). 스타일 편집 패널은 여전히 "현재(current) 항목" 하나만 대상으로 동작 — 다중 항목 스타일 일괄 편집은 이번 범위 밖
- **Ctrl+A 전체 선택은 우측 시리즈 리스트에만** *(2026-09-09 정정)*: 처음엔 좌측 Data 탭 컬럼 리스트에 붙였다가, 사용자가 "좌측이 아니라 우측 데이터 리스트에 붙여달라"고 정정 요청해 위치를 바꿈. `SeriesListWidget.keyPressEvent`에서 `QKeySequence.StandardKey.SelectAll` → `selectAll()`. 좌측 `DraggableColumnList`는 Qt의 `ExtendedSelection`이 원래 Ctrl+A를 기본 지원하므로(내부적으로 `QKeySequence::SelectAll` 처리), 코드를 그냥 지우는 것만으론 동작이 사라지지 않음 — `keyPressEvent`에서 그 키 시퀀스를 명시적으로 삼켜(아무 것도 안 하고 return, `super()`도 호출 안 함) Qt 기본 동작까지 무력화함
- **우측 시리즈 리스트도 드래그 소스로 만들고, 클릭+드래그에 의한 선택 확장은 제거** *(2026-09-09 정정)*: 다중 선택이 되긴 했지만 클릭한 채로 드래그하면 지나간 행이 전부 선택되는 것도 원치 않는 동작이라는 피드백을 받음. `SeriesListWidget.setDragEnabled(True)` + `startDrag()`를 추가(좌측 `DraggableColumnList`와 동일한 패턴)하니 이 문제가 부수적으로 해결됨 — Qt의 `QAbstractItemView`는 `dragEnabled`가 켜져 있으면 이미 선택된 행을 누른 채 움직이는 동작을 "드래그 시작"으로 해석하고, 선택 확장(마퀴 선택)은 `dragEnabled`가 꺼져 있을 때만 일어나는 기본 동작이기 때문. 별도의 억제 코드 없이 두 요구사항(드래그 소스화, 드래그 중 선택확장 금지)이 한 번에 해결됨(실제 마우스 press+move+release 시뮬레이션으로 검증)
- **우측 시리즈 리스트를 다른 subplot에 드래그하면 이동**: 선택된 시리즈(들)의 y_column 이름을 좌측 컬럼 리스트와 같은 규약(개행으로 구분한 plain-text MIME)으로 내보내므로(`SeriesListWidget.startDrag`), `PlotGridWidget.dropEvent()`가 어느 쪽에서 온 드래그든 동일하게 처리함. 표시 텍스트(`"temp  [primary]"`처럼 axis가 붙은 라벨)와 실제 컬럼명이 다르므로, 각 리스트 항목에 컬럼명을 별도 role(`Qt.ItemDataRole.UserRole + 1`)로 함께 저장해 드래그 시 그 값을 꺼내 씀(`refresh_series_list`가 `(series_id, y_column, 표시텍스트)` 튜플을 받도록 시그니처 변경)
- **컬럼이 이미 다른 subplot에 있으면 드롭 시 복사가 아니라 이동**: 어떤 컬럼이 이미 다른 subplot에 시리즈로 들어가 있는 상태에서 그 컬럼을(좌측 컬럼 리스트에서건, 우측 시리즈 리스트에서건) 또 다른 subplot에 드래그&드롭하면, 원래 있던 subplot에서 그 시리즈를 제거하고 드롭한 subplot에 추가함(`MainWindow._on_column_dropped`) — "이 시리즈를 여기로 옮긴다"는 의도로 해석. 판단 기준은 드롭 대상이 아닌 다른 모든 subplot을 통틀어 같은 y_column을 가진 시리즈가 **정확히 하나**일 때만 이동시키는 것 — 이미 여러 subplot에 중복으로 꽂혀 있으면 어느 것을 옮겨야 할지 모호하므로 건드리지 않고 그대로 둠(드롭 대상에는 새 시리즈가 평소처럼 추가됨). 같은 subplot에 다시 드롭하는 경우는 대상 검색에서 애초에 제외되므로 기존의 "같은 subplot 안에서는 중복 추가 허용" 동작은 그대로 유지됨. **이동은 기존 `Series` 객체 자체를 그대로 옮김**(2026-09-09 정정) — 처음엔 지우고 기본 스타일로 다시 만들었는데, 우측 리스트로 드래그하는 경우는 사용자가 이미 색상/축/마커 등을 손봐둔 실제 시리즈를 옮기는 것이므로 스타일이 초기화되면 "이동"이라 부르기 어색함. 지금은 `matches`에서 찾은 기존 `Series` 인스턴스를 그대로 `subplot.add_series()`해 id/색상/선스타일/마커/축 전부 보존(좌측 컬럼 리스트에서 드래그하는 경우도 동일 코드 경로를 타므로 똑같이 스타일이 보존됨). 이동 로직은 `MainWindow._remove_series`가 하던 일을 임의의 subplot에 대해 수행할 수 있도록 `_remove_series_from(subplot, series_id)`로 일반화해서 재사용(`_remove_series`는 이제 `_remove_series_from(self._active_subplot(), series_id)`의 얇은 래퍼)

### 3.7 X축 제목 자동 생략(공유 X축) *(2026-09-09 추가)*
- **그리드의 모든 subplot이 완전히 같은 X 데이터(같은 컬럼 + 같은 offset)를 가지면, 맨 아래 행(row)의 subplot만 X축 제목을 표시하고 나머지는 비움** — 세로로 쌓인 subplot들이 같은 X축을 공유할 때 반복되는 제목만큼 각 subplot의 실제 그래프 영역이 좁아지는 게 아까워서 추가. `MainWindow._shared_x_axis()`가 `project.subplots` 전체를 훑어 컬럼명과 offset이 전부 동일한지 확인(하나라도 다르거나 X 컬럼이 비어있으면 False) → `_effective_x_label(subplot)`이 이 조건이 참이고 자신이 마지막 행(`subplot.row == grid_rows - 1`)이 아닐 때만 빈 문자열을 반환, 그 외에는 평소처럼 `subplot.x_label or subplot.x_column`. `_replot_subplot()`이 매번 자기 자신뿐 아니라 그리드 전체에 대해 `_sync_x_axis_titles()`를 호출해 다시 계산하므로, X 컬럼/offset을 바꾸는 순간(공유가 성립되거나 깨지는 순간) 모든 subplot의 제목이 즉시 갱신됨 — 별도로 "공유 모드"를 저장하거나 켜고 끄는 토글은 없고 항상 현재 상태에서 다시 계산되는 파생값
- **높이가 실제로 늘어나는 원리**: pyqtgraph의 `AxisItem.setLabel(text="")`는 `showLabel(False)`를 유발해 그 축의 `label.setVisible(False)`로 만들고, `AxisItem._updateHeight()`는 라벨이 보이지 않으면 그만큼(`label.boundingRect().height() * 0.8`)을 높이 계산에서 아예 빼고 `setMinimumHeight`/`setMaximumHeight`를 그 값으로 고정함 — 이 앱은 행(row)에 대해 pyqtgraph 기본값인 동일 stretch factor만 쓰고(열처럼 `setColumnFixedWidth`로 폭을 강제 고정하는 로직 없음, `_sync_column_widths` 참고) Qt의 `QGraphicsGridLayout`이 알아서 줄어든 최소높이를 반영해 남는 공간을 다른 subplot들에 재분배하므로, 별도의 높이 계산/고정 코드 없이 `setLabel("bottom", "")` 한 줄만으로 원하는 효과(제목 없는 행은 낮아지고 그만큼 전체적으로 그래프 영역이 늘어남)가 남
...
- **Save Layout / Save Layout As... / Load Layout / Load Layout As...**: subplot 구성(그리드 크기, 각 subplot의 X컬럼/라벨/범례 표시 여부/시리즈 목록과 스타일)을 JSON으로 직렬화. 기본 경로는 CSV 파일명 기반(`<csv 이름>.json`). 컬럼을 이름으로만 참조하는 범용 포맷이라 Load Layout As...로 다른 CSV에도 재사용 가능(없는 컬럼은 자동으로 걸러짐)
  ...
  - **Legend Font Size(Settings 메뉴)도 이제 레이아웃에 포함** *(2026-09-09 추가)*: 원래 `PlotGridWidget._legend_font_pt`로만 존재하는 앱 세션 상태였고 subplot 개별 설정도 아니었던 탓에 저장/복원 대상이 아니었음(사용자가 확인 요청). `ProjectState.legend_font_size`(기본값 9 = Medium, `Settings > Legend Font Size`의 Small/Medium/Large ↔ 7/9/11pt와 동일 기본값)로 프로젝트 상태에 승격해 다른 필드처럼 저장/로드됨. 메뉴에서 값을 바꾸면 `MainWindow._on_legend_font_size_changed`가 `project.legend_font_size`를 갱신하고 `plot_grid.set_legend_font_size()`를 호출 — CSV/레이아웃을 새로 열 때(`_refresh_subplot_selector`)는 반대로 로드된 `project.legend_font_size`를 `plot_grid`에 적용하고 메뉴의 체크 표시도 그 값에 맞춰 동기화(`_sync_legend_font_menu`). 예전에 저장된, 이 키가 없는 레이아웃 파일은 `csv_path`와 같은 방식으로 기본값(9)으로 안전하게 로드됨(하위호환)
  - **더 작은 Legend Font Size 옵션 "Tiny"(5pt) 추가** *(2026-09-09 추가)*: 기존 Small/Medium/Large(7/9/11pt)보다 더 작은 크기가 필요하다는 요청으로 목록 맨 앞에 추가. 메뉴 항목이 `(label, size_pt)` 튜플 목록을 순회하며 `QActionGroup`과 `self._legend_font_actions` 딕셔너리를 그때그때 채우는 구조라, 튜플 하나만 추가하면 나머지(체크 상태 동기화, 레이아웃 저장/복원)는 전부 기존 코드가 그대로 처리함
  - **Tiny에서 글자만 작아지고 항목 간 상하 간격은 안 줄던 문제 수정** *(2026-09-09 추가)*: pyqtgraph의 범례 아이콘(`ItemSample`, 각 항목 왼쪽의 선/마커 견본)은 폰트 크기와 무관하게 항상 20x20px 고정(`pyqtgraph/graphicsItems/LegendItem.py`). 이 앱의 `_resize_legend_to_fit()`은 각 행의 높이를 `max(아이콘 높이, 텍스트 높이)`로 계산하는데, Medium(9pt)까지는 텍스트 높이가 이미 20px 근처/이상이라 아이콘이 병목이 아니었지만, Tiny(5pt)에서는 텍스트가 그보다 작아지면서 고정된 20px 아이콘이 행 높이를 떠받치는 바닥짐이 되어버려 글자만 작아지고 행 간격(사실상 행 높이)은 더 줄지 않았음. `SubplotView`에 `_ScalableItemSample`(`pg.ItemSample` 서브클래스)을 새로 만들어 아이콘 정사각형 크기를 폰트 크기에 비례하도록 함(`_legend_sample_size()`: 9pt=20px 기준으로 비례 계산하되 20px을 절대 넘지 않도록 clamp — Medium/Large는 지금과 완전히 동일, Small/Tiny만 그 아래로 줄어듦). 그리기는 `pg.ItemSample.paint()`가 20x20 기준 좌표에 그리는 그대로 두고 `QPainter.scale()`로 감싸서 스케일만 조정(마커/라인 견본도 같이 비례 축소됨 — 부수효과지만 자연스러움). `LegendItem.addItem()`이 `sampleType(item)`으로만 아이콘을 생성해 크기를 인자로 못 넘기는 구조라, "새로 추가되는 시리즈"의 초기 아이콘 크기는 `_ScalableItemSample.default_size` 클래스 속성(폰트 크기 바뀔 때마다 갱신)에서 가져오고, 이미 추가된 아이콘들은 `set_legend_font_size()`가 순회하며 `sample.set_size()`로 즉시 리사이즈
  - **Y축(주축) 및 이중 Y축(보조축) 확대/축소 범위도 레이아웃에 저장** *(2026-09-09 추가)*: 원래는 CSV/레이아웃을 열 때마다 `_autorange_all_views()`가 항상 데이터 범위로 다시 자동 맞춤해서, 사용자가 수동으로 확대/축소해 둔 Y 범위(주축이든 `right_vb`로 그리는 보조축이든)가 저장되지도, 다시 열었을 때 복원되지도 않았음(사용자 확인 요청). `SubplotConfig`에 `y_range_left`/`y_range_right`(`[min, max]` 또는 저장된 적 없으면 `None`) 필드를 추가하고, Save Layout/Save Layout As... 직전에 `MainWindow._capture_current_y_ranges()`가 각 subplot의 현재 화면 범위(`view.plot_item.vb.viewRange()[1]`, 보조축을 실제로 쓰는 subplot이면 `view.right_vb.viewRange()[1]`도)를 읽어 프로젝트에 채워 넣음 — series가 없는 subplot은 의미 없는 pyqtgraph 기본값(0, 1) 대신 `None`으로 정리. CSV를 열 때(저장된 레이아웃을 자동으로 함께 불러온 경우만) 그리고 Load Layout/Load Layout As...로 불러올 때는 기존처럼 `_autorange_all_views()`로 먼저 데이터에 맞춰 자동 범위를 잡은 **다음**, `_restore_saved_y_ranges()`가 저장된 값이 있는 subplot만 그 위에 덮어써 정확히 복원(저장된 값이 없으면 방금 계산된 autorange 결과 그대로 유지). `SubplotConfig`는 `**sp` 형태로 JSON을 그대로 언패킹해 만들어지므로(`serialization.py::load_project`), 이 키가 없는 예전 레이아웃 파일도 dataclass 기본값(`None`)으로 별도 처리 없이 안전하게 로드됨
