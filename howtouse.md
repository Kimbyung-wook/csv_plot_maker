...
![CSV를 불러온 직후: 좌측에 컬럼 목록이 채워지고, X column이 timestamp로 자동 선택된 상태](docs/images/csv_loaded_columns.png)
...
![altitude_m subplot에 X offset 50을 적용한 모습 -- X축 눈금이 50만큼 밀린 것을 볼 수 있다](docs/images/x_axis_offset.png)
...
![speed_kph(주축, 왼쪽 눈금)와 battery_pct(보조축, 오른쪽 눈금)를 한 subplot에 함께 표시한 예시. 우측 스타일 패널의 Y axis가 Right (secondary)로 설정되어 있다](docs/images/dual_axis_style.png)
...
우측 목록에서 시리즈를 하나 클릭하면 하단에 스타일 패널이 나타납니다(위 5번 스크린샷의 우측 하단 "Series style" 영역 참고).
