...
- 파일 이름 앞의 세모(▾) 부분을 클릭하면 그 파일의 컬럼 목록을 접었다 펼 수 있습니다(▸로 바뀌면 접힌 상태). 열려 있는 파일이 많아 목록이 길어졌을 때, 지금 안 쓰는 파일을 접어서 화면을 정리하는 용도입니다. 접은 상태는 저장되지 않고 그 세션에서만 유지되며, 파일을 닫았다가 다시 열면 항상 펼쳐진 상태로 시작합니다. 접힌 컬럼은 Ctrl+F 검색에도 걸리지 않습니다.
